...
.base:
	...
	# Create FNLGNLHNLFunctions directory in the build directory
	touch build/.base
...
# Add vpath for FNLGNLHNLFunctions directory
vpath %.c FNLGNLHNLFunctions
...
FNLGNLHNL_SOURCE = fNLgNLhNL.o omega_functions.o
HEADERFILES += FNLGNLHNLFunctions/fNLgNLhNL.h FNLGNLHNLFunctions/omega_functions.h
C_SOURCE += $(FNLGNLHNL_SOURCE)

$(FNLGNLHNL_SOURCE): %.o: %.c .base $(HEADERFILES)
	cd $(WRKDIR);$(CC) $(OPTFLAG) $(OMPFLAG) $(CCFLAG) $(INCLUDES) -c ../$< -o $*.o
...
# Update the list of all source files
C_SOURCE += $(addprefix FNLGNLHNLFunctions/, $(FNLGNLHNL_SOURCE))

# Ensure the new source files are compiled
build/%.o: FNLGNLHNLFunctions/%.c .base $(HEADERFILES)
	cd $(WRKDIR); $(CC) $(OPTFLAG) $(OMPFLAG) $(CCFLAG) $(INCLUDES) -c ../$< -o $*.o

# Update the class target to include the new object files
class: $(TOOLS) $(SOURCE) $(EXTERNAL) $(OUTPUT) $(CLASS) $(addprefix build/, $(FNLGNLHNL_SOURCE))
...
# Add the mypython directory to the library path
export LD_LIBRARY_PATH := $(LD_LIBRARY_PATH):$(MDIR)/FNLGNLHNLFunctions
