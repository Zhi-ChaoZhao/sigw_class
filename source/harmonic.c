...
int harmonic_compute_cl(
  ...
  for (index_q=0; index_q < ptr->q_size; index_q++) {
    ...
    if (ppt->has_cl_gwb == _TRUE_) {
      // check if ppm->gwi_scalar == 10000000.
      if (ppm->gwb_source_type != PBH_gwb) {
        printf("Error: ppm->gwb_source_type is not PBH_gwb. Actual value: %d\n", ppm->gwb_source_type);
        assert(0);
      }
      for (index_d1=0; index_d1<phr->f_gwb_num; index_d1++) {
        ...
        class_call(primordial_PBH_omega_ng_over_total_FNL_GNL_HNL(ppm,
                                         phr->f_gwb[index_d1],
                                         &tmp_gwi_scalar),
                    ppm->error_message,
                    ppm->error_message);
        ppm->gwi_scalar = tmp_gwi_scalar;
        // printf("ppm->gwi_scalar = %e\n", ppm->gwi_scalar);

        if (_scalars_) { // ppm->gwi_scalar / (4. - n_gwb) is 3.11 * 3/5 of 2305.01602v2
