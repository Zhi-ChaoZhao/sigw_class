...
int primordial_omega_gw_at_f(
  ...
  if (ppm->gwb_source_type == PBH_gwb) {
    ...
    /* direct computation */
    // printf("This call is from primordial_omega_gw_at_f, line 4300\n");
    class_call(primordial_PBH_omega_gw_FNL_GNL_HNL(ppm,
                                        exp(lnf),
                                        output),
                ppm->error_message,
                ppm->error_message);
...
int primordial_init(
  ...
  /** - deal with the case of external calculation of \f$ P_k \f$*/
    ...
    class_test(ppt->has_bi == _TRUE_ || ppt->has_cdi == _TRUE_ || ppt->has_nid == _TRUE_ || ppt->has_niv == _TRUE_,
               ppm->error_message,
               "external Pk module cannot work if you ask for isocurvature modes except 'gwi' (but that could be implemented easily in the future!)");
    ...
    class_stop(ppm->error_message,"primordial spectrum type not recognized");
  ...
  if (ppm->has_OmGW == _TRUE_) {
    ...
    /** - deal with case of analytic GWB */
    ...
    /** - deal with the case of GWB from primordial balck holes */
    else if (ppm->gwb_source_type == PBH_gwb) {
      if (ppm->primordial_verbose > 0)
        printf(" (GWB generated by PBH)\n");

      class_call(primordial_PBH_gwb_init(ppr,pba,ppt,ppm),
                ppm->error_message,
                ppm->error_message);
    }

    /** - deal with the case of GWB from phase transitions */
    else if (ppm->gwb_source_type == PT_gwb) {
      if (ppm->primordial_verbose > 0)
        printf(" (GWB generated by PT)\n");

      /** - calculate \f$ \Omega_\mathrm{GW} \f$ */
      if (ppm->has_OmGW == _TRUE_) {
        for (index_f = 0; index_f < ppm->lnf_size; index_f++) {
          f = exp(ppm->lnf[index_f]) / ppm->fPT_star;

          OmGW = ppm->OmegaPT_star * pow(f, ppm->nPT_1) * pow(1 + pow(f, ppm->deltaPT), (ppm->nPT_2 - ppm->nPT_1) / ppm->deltaPT);
  ...
  printf("ppm->primordial_spec_type = %d\n",ppm->primordial_spec_type);
  if (ppm->primordial_spec_type != analytic_Pk) {
    printf("This is primordial_initaaaaaaaaaaa\n");
    assert(0);
...
int primordial_analytic_spectrum_init(
  ...
  printf("This is primordial_analytic_spectrum_init\n");
  // assert(0);  
...
int primordial_inflation_potential(
  ...
  printf("This is primordial_inflation_potential, line 1521\n");
  assert(0);
...
int primordial_inflation_hubble(
  ...
  printf("This is primordial_inflation_hubble, line 1531\n");
  assert(0);
...
int primordial_inflation_indices(
  ...
  printf("This is primordial_inflation_indices, line 1608\n");
  assert(0);
  int index_in;
...
int primordial_inflation_solve_inflation(
  ...
  printf("This is primordial_inflation_solve_inflation, line 1688\n");
  assert(0);
  /** Summary: */
  /** - define local variables */
  double * y;
...
int primordial_inflation_analytic_spectra(
  ...
  printf("This is primordial_inflation_analytic_spectra, line 2074\n");
  assert(0);
  double * y;
  double * dy;
  int index_k;
...
int primordial_inflation_spectra(
  ...
  printf("This is primordial_inflation_spectra, line 2152\n");
  assert(0);
  int index_k;
...
int primordial_inflation_one_wavenumber(
  ...
  printf("This is primordial_inflation_one_wavenumber, line 2252\n");
  assert(0);
  double k;
  double curvature,tensors;
  double * y;
...
int primordial_inflation_one_k(
  ...
  printf("This is primordial_inflation_one_k, line 2345\n");
  assert(0);
  /** Summary: */
...
int primordial_inflation_find_attractor(
  ...
  printf("This is primordial_inflation_find_attractor, line 2510\n");
  assert(0);
...
int primordial_inflation_evolve_background(
  ...
  printf("This is primordial_inflation_evolve_background, line 2649\n");
  assert(0);
  struct primordial_inflation_parameters_and_workspace pipaw;
  struct generic_integrator_workspace gi;
...
int primordial_inflation_check_potential(
  ...
  printf("This is primordial_inflation_check_potential, line 2973\n");
  assert(0);
...
int primordial_inflation_check_hubble(
  ...
  printf("This is primordial_inflation_check_hubble, line 3017\n");
  assert(0);
...
/**
 * This routine returns the GWB energy density in the simple analytic case with
 * amplitude, tilt and running.
 *
 * @param ppm            Input/output: pointer to primordial structure
 * @param f              Input: frequency in same units as pivot scale, i.e. in Hz
 * @param OmGW           Output: GWB enery density A (f/f_pivot)^(n+...)
 * @return the error status
 */
...
int primordial_analytic_omega_gw(
  ...
  printf("This is primordial_analytic_omega_gw\n");
  assert(0);
...
int primordial_inflationary_gwb_init(
  ...
  printf("This is primordial_inflationary_gwb_init\n");
  assert(0);
 ...
 * Compare to primordial_external_spectrum_init
 *
...
int primordial_external_gwb_init(
  ...
  /** Summary: */
  printf("This is primordial_external_gwb_init\n");
  assert(0);
  char arguments[_ARGUMENT_LENGTH_MAX_];
  char line[_LINE_LENGTH_MAX_];
  char command_with_arguments[2*_ARGUMENT_LENGTH_MAX_];
...
/**
 * This routine calculates the GWB generated by primordial black holes (PBHs).
 *
 * @param ppr  Input: pointer to precision structure
 * @param pba  Input: pointer to background structure
 * @param ppt  Input: pointer to perturbation structure
...
int primordial_PBH_gwb_init(
  ...
  printf("This is primordial_PBH_gwb_init\n");
  // assert(0);
  int index_f;
  double OmGW, aHtau2;
  ...
  class_test(ppm->has_OmGW == _FALSE_,
             ppm->error_message,
             "You try to calculate the PBH_gwb without having a GWB, this should not happen!");

  // class_test(ppm->f_pivot > ppm->f_star,
  //            ppm->error_message,
  //            "At the observed GWB frequency f_pivot=%g Hz there is no GWB generated by PBHs with an enhancement scale at f_star=%g Hz.",
  //            ppm->f_pivot, ppm->f_star);

  /** - calculate the prefactor of OmGW */
  // aHtau2 = pba->conformal_age*pba->conformal_age * pba->H0*pba->H0;
  // ppm->prefactor_OmGW = 1. / aHtau2 * ppm->A_star*ppm->A_star / 15552.;
  // Z-C.Z : we have commented out the above prefactor of OmGW, since we will use Omega.ob now, from mypython/plotfunc.py.
  ...
  // ppm->gwi_scalar = 3./5. * 8. * ppm->f_NL; // Eq.3.11 * Eq.3.12, but without Rg and (4-nGWb)

  // 这里比较麻烦，这不是一个 double, 而是一个与 f 有关的东西。。。
  ppm->gwi_scalar = 20000000.; // it is Omega.total in mypython/plotfunc.py. We do not use the value given here, so it is ok to be any value.
...
/**
 * This routine returns the GWB energy density in the case of PBH_gwb.
 *
 * @param ppm            Input/output: pointer to primordial structure
 * @param f              Input: frequency in same units as pivot scale, i.e. in Hz
 * @param OmGW           Output: GWB enery density A (f/f_pivot)^(n+...)
...
// the following line is commented out, since we will use Omega.ob now, from mypython/plotfunc.py.
...
int primordial_PBH_omega_gw_FNL_GNL_HNL(  // Note , the h^2 has been removed in the python file.
                          struct primordial * ppm, 
                          double f,
                          double * OmGW
                          ) {
double f_over_fstar;
f_over_fstar = f / ppm->f_star;
// Param: f_over_fstar, 1.0, ppm->A_star, ppm->f_NL


zOpar myzOpar_my;
myzOpar_my.sigma = 1.0;
myzOpar_my.k_ = f_over_fstar;
myzOpar_my.A = ppm->A_star;
myzOpar_my.nu_s = 1.0;
myzOpar_my.FNL = ppm->f_NL * 3.0 / 5.0;
...
double h = 0.6736;
double ob_value = ob(myzOpar_my) / h /h;

double B = evaluate_spline_B(f);

*OmGW = ob_value * B;
return _SUCCESS_;
}

int primordial_PBH_omega_ng_over_total_FNL_GNL_HNL(  // this is gwi_scalar
                          struct primordial * ppm, 
                          double f,
                          double * gwi_scalar
                          ) {
...
double ng1_value = ng1(myzOpar_my);
double total_value = total(myzOpar_my);

double ng1_over_total = ng1_value / total_value;

*gwi_scalar = ng1_over_total;
return _SUCCESS_;
}
