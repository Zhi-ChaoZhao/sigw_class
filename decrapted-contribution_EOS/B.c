...
double f_ggf[DATA_POINTS];
double B_ggf[DATA_POINTS];
...
void initialize_interpolation() {
    ...
    spline = gsl_spline_alloc(gsl_interp_linear, DATA_POINTS);
    gsl_spline_init(spline, f_ggf, B_ggf, DATA_POINTS);
}
...
double interpolate(double x) {
    ...
    if (x >= f_ggf[DATA_POINTS - 1]) {
        return B_ggf[DATA_POINTS - 1];
    }
