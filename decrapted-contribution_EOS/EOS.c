...
double geff_rho_l(double T, double p1, double p2) {
...
double geff_s_l(double T, double p1, double p2) {
  double me = 511e-6, mmu = 0.1056, mpi0 = 0.135, mpip = 0.140, m1 = 0.5,
         m2 = 0.77, m3 = 1.2, m4 = 2.0;
...
double geff_rho(double T) {
  if (T < 0.12)
    return geff_rho_l(T, 1, 1);
  return geff_rho_h(T);
}
double geff_s(double T) {
  if (T < 0.12)
    return geff_s_l(T, 1, 1);
  return geff_s_h(T);
}

int main() {
  double T;
  for (T = 1e-6; T < 1e15; T *= 1.1) {
    printf("%e %e %e\n", T, geff_rho(T), geff_s(T));
  }
  return 0;
}
